...
public record BoardBbsCreateVO(
        @NotBlank(groups = {BoardBbsValidationGroup.Create.class}, message = "Bbs의 Path는 공백일 수 없습니다.")
        @Pattern(groups = {BoardBbsValidationGroup.Create.class}
                , regexp = "^[a-z_]+$"
                , message = "Bbs의 Path는 소문자와 언더스코어(_)만 포함할 수 있습니다."
        )
        @Size(groups = {BoardBbsValidationGroup.Create.class}
                , min = 1, max = 10
                , message = "Bbs의 Path는 1글자 이상 10자리 이하여야 합니다."
        )
        String path,

        @NotBlank(groups = {BoardBbsValidationGroup.Create.class}, message = "Name은 공백일 수 없습니다.")
        @Size(groups = {BoardBbsValidationGroup.Create.class}
                , min = 1, max = 10
