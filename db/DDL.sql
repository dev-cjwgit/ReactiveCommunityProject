...
-- 테이블 rcdb.RC_BOARD_FILE 구조 내보내기
CREATE TABLE IF NOT EXISTS `RC_BOARD_FILE` (
  `UID` bigint(20) NOT NULL AUTO_INCREMENT,
  `BOARD_UID` bigint(20) NOT NULL,
  ...
  `UPDATED_AT` datetime NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`UID`),
  KEY `BOARD_FILE_FILE_UID_FK` (`FILE_UID`),
  KEY `BOARD_UID` (`BOARD_UID`),
  CONSTRAINT `BOARD_FILE_BOARD_UID_FK` FOREIGN KEY (`BOARD_UID`) REFERENCES `RC_BOARD` (`UID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `BOARD_FILE_FILE_UID_FK` FOREIGN KEY (`FILE_UID`) REFERENCES `RC_FILE` (`UID`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=86 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- 내보낼 데이터가 선택되어 있지 않습니다.

-- 테이블 rcdb.RC_FILE 구조 내보내기
...
CREATE TABLE IF NOT EXISTS `RC_USER_RESOURCE` (
  ...
  `UPDATED_AT` datetime NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`UID`) USING BTREE,
  UNIQUE KEY `TYPE_METHOD_PATTERN` (`METHOD`,`PATTERN`) USING BTREE
...
CREATE TABLE IF NOT EXISTS `RD_ROLE_RESOURCE` (
  ...
  `UPDATED_AT` datetime NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`UID`),
  UNIQUE KEY `LEVEL_UID_RESOURCE_UID` (`LEVEL_UID`,`RESOURCE_UID`),
  KEY `ROLE_RESOURCE_RESOURCE_UID_FK` (`RESOURCE_UID`),
  KEY `ROLE_RESOURCE_LEVEL_UID_FK` (`LEVEL_UID`),
